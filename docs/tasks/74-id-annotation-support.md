...
14. ✅ **Audit the unit test suite against the Cedar annotations documentation.**
    ...
    - **Gap (now closed)**: docs claim 3 (value-less annotation ≡ empty-string value) wasn't exercised, and the behavior is non-obvious. cedar-py treats an empty `@id` value as a missing display id and falls back to the parser-generated id — `@id` is a labeling convention and an empty display id is unhelpful for logs/lookups. This is a deliberate cedar-py choice and diverges from cedar-policy-cli (which would rename the policy to `""`). Convergence: added `test_id_annotation_empty_value_falls_back_to_parser_id` covering both `@id` (no parens) and `@id("")` via `subTest`.
