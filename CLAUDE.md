...
- **Benchmarks:**
  ...
  - **Benchmark results are load-sensitive — run on a quiet machine and read failures diagnostically.** Concurrent work (a cargo build, another benchmark) inflates every median roughly uniformly (+5–8% observed); even quiet, ambient drift vs an older baseline runs +2–4%. A *uniform* shift that includes benchmarks the change cannot touch (e.g. no-schema benchmarks for a schema-path change) indicates contamination/drift, not a regression — use the untouched benchmarks as the control group before trusting a FAIL.
