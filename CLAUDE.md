...
- **The `cedarpy.pst` mirror is a named engine-bump step.** `cedarpy.pst` hand-mirrors cedar's `#[non_exhaustive]` pst enums, so an engine bump that adds a variant needs: the Rust converter match arms in `src/lib.rs`, the node types / `Literal` members in `cedarpy/pst.py`, and a regenerated `tests/unit/pst_variants.json` (`python tests/unit/test_pst_variant_coverage.py`). The variant-coverage test fails naming the drifted variant; until the mirror is updated, unmodelled syntax raises `ValueError` at runtime.
