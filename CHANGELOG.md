...
- `tpe_authorize(principal, action, resource, policies, entities, schema, context=None, verbose=False)` performs type-aware partial evaluation (TPE) on a request whose principal or resource identity is unknown. It is a separate entry point from `is_authorized_partial`, which it does not call or change, and its residuals are typed `cedarpy.pst.Template` nodes rather than JSON. `TpeAuthzResult.reauthorize(request, entities=None)` binds the unknowns and decides by evaluating only the residuals, and Cedar checks the concrete request against the partial one first, so a contradicting request raises. `tpe_reauthorize(...)` is the same operation as a free function. `PartialEntities.from_json(...)` accepts an entity document in which an entity's `attrs`, `parents`, or `tags` may be absent to mean unknown, so a caller can decide what to load before loading it. See the README for the argument forms and the `context` contract. Enables the `tpe` Cargo feature, which adds no dependencies.

- `pst.ResidualError`, a node for a residual subexpression TPE knows will error if it is evaluated. Cedar has no surface syntax for it, so it appears only inside a TPE residual. It exists only because this build enables the `tpe` feature.
