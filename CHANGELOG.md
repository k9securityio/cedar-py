...
- `pst.entity_uids(node)` collects every entity uid named anywhere under a node, for deciding what to load before evaluating. It takes a node, or a mapping or tuple of nodes such as a `PolicySet`'s `templates`. Anything it cannot walk raises `TypeError` rather than returning an empty result, so passing a `PolicySet` handle by mistake cannot look like "this policy names no entities". A TPE residual is a `pst.Template`, so this is also how a caller finds out what is still needed before an evaluation can finish. Note that `residual_policies` and `residual_policy_set` name different entities, and the handle needs `to_pst()` first.

- `tpe_authorize(principal, action, resource, policies, entities, schema, context=None, verbose=False)` performs type-aware partial evaluation (TPE) on a request whose principal or resource is known only by type. It requires a schema, and its residual policies are typed `cedarpy.pst.Template` nodes rather than JSON. `permits` and `forbids` stay separate, each a `TpeClassification` of residual, true, false, and error policy ids. It is a separate entry point from `is_authorized_partial`, which it neither calls nor changes. `TpeAuthzResult.reauthorize(request, entities=None)` binds the unknowns and decides by evaluating only the residuals, and Cedar checks the concrete request against the partial one first, so a contradicting request raises. `tpe_reauthorize(...)` is the same operation as a free function. `PartialEntities.from_json(...)` accepts an entity document in which an entity's `attrs`, `parents`, or `tags` may be absent to mean unknown, so a caller can decide what to load before loading it. Enables the `tpe` Cargo feature, which adds no dependencies. See the [Type-Aware Partial Evaluation Guide](docs/guides/type-aware-partial-evaluation-guide.md) for the argument forms and the `context` contract. ([#109](https://github.com/k9securityio/cedar-py/pull/109)). Thanks [@h0rv](https://github.com/h0rv)!

- `pst.ResidualError`, a node for a residual subexpression TPE knows will error if it is evaluated. Cedar has no surface syntax for it, so it appears only inside a TPE residual, and `PolicySet.from_pst` raises `TypeError` on a node tree containing one. It exists only because this build enables the `tpe` feature.
