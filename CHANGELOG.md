...
- `policies_to_pst(policies)` parses Cedar policy text into typed `cedarpy.pst` nodes (frozen dataclasses, one per `cedar_policy::pst` node kind), matching a real Cedar structure a consumer can pattern-match on instead of an untyped tree keyed by string operators. Static policies and unlinked templates only; a partial-evaluation residual from `is_authorized_partial` cannot be represented this way, because PST's own policy type validates every clause as it is built and rejects any containing an unresolved `unknown(...)` node ([#107](https://github.com/k9securityio/cedar-py/issues/107)). Requires cedar-policy 4.11.0+, bundled by [#106](https://github.com/k9securityio/cedar-py/pull/106). Closed sets stay closed on the Python side: the operator names are `Literal` aliases (`UnaryOpName`, `BinaryOpName`), as are `Template.effect`, `Var.name`, and `Slot.name`, so a `match` over one is statically exhaustive; Cedar's `Bool` and `Long` literals are separate `BoolLit` / `LongLit` nodes, since `bool` is a subclass of `int` and one node could not distinguish them; entity type namespaces stay structured as `EntityType(basename, namespace)` rather than a `"::"`-joined string; `HasAttr.attrs` rejects the empty path its Rust `NonEmpty` counterpart forbids; and every collection field is fully parameterized (`tuple[Expr, ...]`, not bare `tuple`). Nodes are `slots=True` (no per-instance `__dict__`, since a policy set is many small nodes), are hashable, and mapping-valued fields are declared as read-only `Mapping`s holding a `FrozenMap` (a mutation-rejecting `dict` subclass), so a write is caught statically and at runtime while `dataclasses.asdict` and `json.dumps` keep working.

- `PolicySet.from_pst(nodes)` and `PolicySet.to_pst()` complete the round trip, so a policy set can be taken apart as nodes, inspected or rewritten, and turned back into a handle the engine authorizes against. `policies_to_pst(text)` is now `PolicySet.from_str(text).to_pst()`. Every one of the 7,551 corpus policies that converts to nodes rebuilds into an identical node tree. Passing anything other than a `cedarpy.pst` node raises `TypeError`, and nodes that do not form a valid set (a template link naming a template that is not present, say) raise `ValueError`.

- `pst.entity_uids(node)` collects every entity uid named anywhere under a node, at any depth, which is what a caller needs to decide what to load before evaluating. It accepts a node, or a mapping or tuple of nodes such as a `PolicySet`'s `templates`. Anything it cannot walk raises `TypeError` rather than reporting an empty result, so passing the engine's own `PolicySet` handle by mistake cannot look like "this policy names no entities".

- A test that audits `cedar_policy::pst`'s enum variants against a snapshot in the repo. Those enums are all `#[non_exhaustive]`, so no exhaustive `match` is possible from outside the crate and the compiler cannot report a variant cedarpy does not model. The audit reads the variant names, and the cargo feature each one needs, out of the `cedar-policy-core` source that `Cargo.lock` resolves to, so a cedar-policy bump that adds, removes or renames a variant fails CI and names it. The corpus suite also runs all 7,600 fuzzer-generated policies through the conversion and back, which catches a regression in what is already modelled.
