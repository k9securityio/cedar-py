...
- `tpe_authorize(principal, action, resource, policies, entities, schema, context=None, verbose=False)` performs type-aware partial evaluation (TPE) on a request whose principal and/or resource identity is unknown. Separate entry point from `is_authorized_partial`, which it does not call or change. TPE's residuals are typed `cedarpy.pst.Template` nodes, unlike `is_authorized_partial`'s. `permits`/`forbids` stay separate, each a `TpeClassification` of residual/true/false/error policy ids. `principal`/`resource` take a concrete entity (surface-syntax string or `{"type", "id"}` dict) or a type whose id is unknown (bare type string, `pst.EntityType`, or a dict carrying only `type`); `action` must be concrete. `context=None` means the context is **unknown** and policies reading it stay residual, matching `PartialRequest` and `is_authorized_partial` — `{}` is a known-empty context. `TpeAuthzResult.reauthorize(request, entities=None)` binds the unknowns and decides by evaluating only the residuals, with Cedar checking the concrete request and entities for consistency against the partial ones first, so a contradicting request raises instead of returning an unsanctioned decision; `tpe_reauthorize(...)` is the same operation as a free function. `residual_policy_set` returns every residual as a reusable `PolicySet` handle. `PartialEntities.from_json(...)` accepts an entity document in which an entity's `attrs`, `parents`, or `tags` may be absent to mean unknown, so a caller can decide what to load before loading it. Enables the `tpe` Cargo feature; adds no dependencies, checked against the resolved lockfile.
