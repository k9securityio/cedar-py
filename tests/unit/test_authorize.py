...
class AuthorizeTestCase(unittest.TestCase):
    ...
    def test_principal_action_resource_may_be_string_or_dict(self):
        """Each EUID field accepts either Cedar surface syntax or
        ``{"type":..., "id":...}``. Mixing forms within a single request
        is allowed."""
        string_form = {
            "principal": 'User::"bob"',
            "action": 'Action::"view"',
        ...
        dict_form = {
            ...
            "resource": {"type": "Photo", "id": "1234-abcd"},
        }
        mixed_form = {
            "principal": {"type": "User", "id": "bob"},
            "action": 'Action::"view"',
        ...
        expect_allow = AuthzResult({
            "decision": "Allow",
            "diagnostics": {"reason": ["policy0"], "errors": []},
        })
        for request in [string_form, dict_form, mixed_form]:
            actual = is_authorized(request, self.policies["bob"], self.entities)
            self.assert_authz_responses_equal(expect_allow, actual)

    def test_dict_form_euid_accepts_ids_unrepresentable_in_surface_syntax(self):
        """Cedar's surface syntax rejects entity ids containing certain
        characters such as embedded newlines ('needs to be normalized');
        the structured ``{type, id}`` form sidesteps that constraint by
        routing through ``EntityUid::from_json``, matching cedar-java's
        request API. Entity ids generated by the upstream fuzzer corpus
        rely on this path."""
        # Embedded newline — accepted by from_json, rejected by surface parser.
        weird_id = "J\nK"
        policies = 'permit(principal, action, resource);'
        entities = [
            {"uid": {"type": "User", "id": weird_id}, "attrs": {}, "parents": []},
            {"uid": {"type": "Photo", "id": weird_id}, "attrs": {}, "parents": []},
        ]
        request = {
            "principal": {"type": "User", "id": weird_id},
            "action": {"type": "Action", "id": "view"},
            "resource": {"type": "Photo", "id": weird_id},
            "context": {},
        }
        actual = is_authorized(request, policies, entities)
        self.assertEqual(Decision.Allow, actual.decision)

        # The same id can't be expressed as a Cedar surface-syntax string —
        # the parser rejects it as non-normalized, producing NoDecision.
        request_str_form = dict(request)
        request_str_form["principal"] = f'User::"{weird_id}"'
        actual_str = is_authorized(request_str_form, policies, entities)
        self.assertEqual(Decision.NoDecision, actual_str.decision)
        self.assertTrue(actual_str.diagnostics.errors,
                        "expected at least one error from the surface-syntax parse failure")
